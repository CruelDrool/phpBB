...
class acp_bbcodes
{
			...
			case 'create':
				...
				if (!$warn_text || confirm_box(true))
				{
					...
					if (($action == 'modify' && strtolower($data['bbcode_tag']) !== strtolower($row['bbcode_tag'])) || ($action == 'create'))
					{
						...
						if (($info['test'] ?? false) || in_array(strtolower($data['bbcode_tag']), $hard_coded) || (preg_match('#\[/([^[]*)]$#', $bbcode_match, $regs) && in_array(strtolower($regs[1]), $hard_coded)))
						{
	...
	*/
	function build_regexp(&$bbcode_match, &$bbcode_tpl)
	{
		...
		if (preg_match_all('/\{(' . implode('|', array_keys($tokens)) . ')[0-9]*\}/i', $bbcode_match, $m))
		{
			foreach ($m[0] as $n => $token)
			{
				...
				if (preg_match_all('/(?<!\\\\)\$([0-9]+)/', $replace, $repad))
				{
					...
					$replace = preg_replace_callback('/(?<!\\\\)\$([0-9]+)/', function($match) use ($pad) { return '${' . ($match[1] + $pad) . '}'; }, $replace);
					$pad = $repad;
				}
		...
		$fp_match = preg_replace_callback('#\[/?' . $bbcode_search . '#i', function($match) { return strtolower($match[0]); }, $fp_match);
		$fp_replace = preg_replace_callback('#\[/?' . $bbcode_search . '#i', function($match) { return strtolower($match[0]); }, $fp_replace);
		$sp_match = preg_replace_callback('#\[/?' . $bbcode_search . '#i', function($match) { return strtolower($match[0]); }, $sp_match);
		$sp_replace = preg_replace_callback('#\[/?' . $bbcode_search . '#i', function($match) { return strtolower($match[0]); }, $sp_replace);
